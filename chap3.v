...
Definition exist_lt_not_exist {P} (DP : isDecidableFamily nat P) :
  (Σ (n : nat), P n)
  → (Σ (n : nat), (P n * ∀ m, m < n → notT (P n))%type).
Proof.
intros (n, pn).
bbb.

Definition PT_exist_lt_not_exist {P} (DP : isDecidableFamily nat P) :
  ∥(Σ (n : nat), P n)∥
  → ∥(Σ (n : nat), (P n * ∀ m, m < n → notT (P n))%type)∥.
Proof.
intros p.
...
assert (q : notT (∀ n, notT (P n))).
 ...
 assert (s : ∀ n A, notT (P n → A n) → P n * notT (A n)).
  ...
  assert (dna : ∀ n, notT (notT (P n)) → P n).
   intros n t.
