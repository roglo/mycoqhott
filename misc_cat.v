...
Definition fm_set : alphabet → Set_type :=
  λ fmt,
  existT _ (list (fm_type fmt))
    (areSet.isSet_list (fm_eq_refl fmt) (fm_is_set fmt)).

Definition kleene_closure (fmt : alphabet) :=
  {| m_set := fm_set fmt;
     m_op a b := a ++ b;
     m_unit := [];
...
Definition ins_gen {fmt} : fm_type fmt → m_type (kleene_closure fmt) :=
  λ a, [a].
...
Theorem UMP_of_free_monoid :
  ∀ (A : alphabet),
  { i : fm_type A → m_type (kleene_closure A) &
