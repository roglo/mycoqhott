...
apply IHd in Hdn.
apply Nat.mod_divide in Hdn; [ | apply fact_neq_0 ].
destruct Hdn as (c, Hc).
rewrite Nat_fact_succ, Hc.
...
rewrite Nat.mul_assoc; f_equal.
...
assert (H : Nat.divide (S d) c). {
  exists (fact n / fact (d + 1)).
  rewrite Nat.add_1_r, Nat_fact_succ.
  rewrite Hc.
